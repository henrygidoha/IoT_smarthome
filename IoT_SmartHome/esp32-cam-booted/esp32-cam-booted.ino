...
uint8_t *snapshot_buf; //points to the output of the capturee
...
/* MQTT loop ------------------------------------------------------- */
  ...
  client.loop();
  //read DHT11
  if(millis()-timeUpdata>3000){
    //Tạo list để gửi tin nhắn với các labels khác nhau
    DynamicJsonDocument doc_1(1024);
    doc_1["human"] = count;
    char mqtt_message[128];
    serializeJson(doc_1,mqtt_message);

    //String message = Serial.readString();
    publishMessage("esp32-cam", mqtt_message, true);
    timeUpdata=millis();
  }
/* ----------------------------------------------------------------------------------- */
}
